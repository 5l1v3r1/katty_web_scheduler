defmodule Katty.Accounts do
  ...
  alias Katty.Accounts.Account
  alias Katty.Accounts.Membership
  alias Katty.Accounts.Member
  alias Katty.Accounts.Invitation

  def paginate_accounts(params \\ %{}) do
    Katty.Pagination.paginate_records(Account, params)
  end
  ...
  alias Katty.Accounts.InvitationNotifier
  ...
  def invite_member({:ok, %{email: email} = _invitation} = result) do
    url =  Routes.user_session_url(KattyWeb.Endpoint, :new)
